...
for (var i = 0; i < datas.length; i++) { // 外面的for循环管行 tr
  ...
  // 3. 创建有删除两个字的单元格 
  var td = document.createElement('td');
  td.innerHTML = '<a href="javascript:;"onclick="MyDelete_1(this)">删除</a>' +
    '<a href = "javascript:;"onclick = "MyUpdate_1(this)">修改</a>';
  tr.appendChild(td);
}

// 点击按钮
var button_1 = document.getElementById("modal_button_1");
...
function MySave_1(btn) {
  ...
  thistd.innerHTML = "<a href='javascript:;'onclick='MyDelete_1(this)'>删除</a>" +
    "<a href = 'javascript:;'onclick='MyUpdate_1(this)'>修改</a>";
}

function MyDelete_1(btn) {
  let tr = btn.parentNode.parentNode;
  if (confirm("确认删除该条记录吗？")) {
    document.getElementById('tab1').removeChild(tr);
  }
}

function MyUpdate_1(btn) {
  let thistd = btn.parentNode;
  let idtd = thistd.previousSibling;
  let phonetd = idtd.previousSibling;
  let nametd = phonetd.previousSibling;
  ...
  thistd.innerHTML = "<a href='javascript:;'onclick='MySave_1(this)'>保存</a>";
}